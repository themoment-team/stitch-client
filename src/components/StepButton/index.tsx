...
const StepButton = ({ variant, onClick, children, disabled = false }: StepButtonProps) => {
  ...
  const variantStyles = {
    ...
    next: "rounded-2xl bg-background px-6 py-3 text-base font-semibold text-ink sm:px-8 sm:py-4 sm:text-lg",
  };

  const nextStateStyles = disabled
    ? "opacity-40 shadow-none"
    : "shadow-[6px_6px_14px_var(--neu-dark),-6px_-6px_14px_var(--neu-light)] hover:shadow-[3px_3px_8px_var(--neu-dark),-3px_-3px_8px_var(--neu-light)] active:shadow-[inset_4px_4px_10px_var(--neu-dark),inset_-4px_-4px_10px_var(--neu-light)]";
